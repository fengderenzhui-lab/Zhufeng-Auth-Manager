...
基于 PHP 8.2+ / Laravel 11 / MySQL 8.0+ 的自托管授权码管理平台，提供**授权码全生命周期**、**一机一码**、**强制在线心跳**、**管理员分级**、**登录安全**、**审计日志**与**接口风控**，项目包含**后端 REST API** 与**双主题管理后台**（深色/浅色，Laravel Blade + 原生 JS + 本地化 Chart.js，无构建链、无 CDN），支持 **Docker Compose 一键部署**（entrypoint 全自动密钥初始化）。
